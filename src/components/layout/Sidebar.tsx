  ...
  return <aside className={`h-screen flex flex-col bg-[#172B4D] transition-all duration-200 relative ${isCollapsed ? 'w-[60px]' : 'w-[260px]'}`}>
      ...
      <div className="px-[16px] py-[16px] flex items-center justify-between border-b border-white/10">
        <div className={`flex items-center gap-3 ${isCollapsed ? 'justify-center w-full' : ''}`}>
          ...
          {!isCollapsed && <span className="text-[15px] font-semibold text-white">AM PM</span>}
        </div>
      </div>
      ...
      <nav className="py-[4px]">
        ...
        return <NavLink key={item.to} to={item.to} className={`flex items-center gap-3 px-[16px] py-[10px] mx-[8px] my-[2px] rounded-[3px] text-[14px] transition-colors duration-150 cursor-pointer ${isActive ? 'bg-[#0052CC] text-white' : 'text-white/80 hover:bg-white/10 hover:text-white'} ${isCollapsed ? 'justify-center px-[8px]' : ''}`} title={isCollapsed ? item.label : undefined}>
              <item.icon className={`w-[18px] h-[18px] flex-shrink-0 ${isActive ? 'opacity-100' : 'opacity-70'}`} strokeWidth={2} />
      ...
      {/* Quick Stats Section */}
      {!isCollapsed && <div className="px-[12px] mt-[16px]">
          <button onClick={() => setIsStatsExpanded(!isStatsExpanded)} className="flex items-center justify-between w-full px-[8px] py-[6px] text-[11px] uppercase tracking-wider text-white/50 hover:text-white/70 transition-colors">
            <span>Quick Stats</span>
            <ChevronDown className={`w-[14px] h-[14px] transition-transform ${isStatsExpanded ? '' : '-rotate-90'}`} />
          </button>
          
          {isStatsExpanded && <div className="space-y-[4px] mb-[8px]">
              {/* Summary Row */}
              ...
              <div className="mt-[8px] pt-[8px] border-t border-white/10">
                ...
                {quickStats.map(stat => <div key={stat.label} className="flex items-center gap-[10px] px-[10px] py-[6px] rounded-[3px] hover:bg-white/5 transition-colors cursor-pointer">
              ...
              <div className="mt-[8px] pt-[8px] border-t border-white/10">
                ...
                <div className="flex items-center justify-between px-[10px]">
                  <div className="flex -space-x-1">
                    {topMembers.map(member => <div key={member.id} className="w-[22px] h-[22px] rounded-full text-[8px] text-white border-2 border-[#172B4D] flex items-center justify-center font-semibold" style={{
      ...
      <div className="px-[16px] py-[12px] border-t border-white/10">
        <div className={`flex items-center gap-[12px] ${isCollapsed ? 'justify-center' : ''}`}>
          <div className="w-[36px] h-[36px] rounded-full text-[13px] text-white flex-shrink-0 flex items-center justify-center font-semibold" style={{
