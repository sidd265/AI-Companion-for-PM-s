      ...
      <div className="w-notion-chat-sidebar border-r border-notion-border bg-[#FAFAF9] flex flex-col">
        <div className="p-[12px] px-[8px]">
          <button
            ...
            className="w-full notion-btn-secondary flex items-center justify-center gap-2"
          >
            ...
            <div key={date} className="mb-[16px]">
              ...
              {convs.map(conv => (
                <div
                  ...
                  className={`group flex items-center justify-between px-[8px] py-[6px] rounded-[4px] cursor-pointer transition-colors duration-150 ${
                ...
                >
                  ...
                  <button
                    ...
                    className="opacity-0 group-hover:opacity-100 p-[4px] hover:bg-notion-border rounded-[4px] transition-opacity duration-150"
                  >
                  ...
                  <div className="space-y-[16px]">
                    <AnimatePresence>
                      {activeConversation.messages.map((message) => (
                        ...
                        >
                          {message.role === 'user' ? (
                            <div className="bg-notion-sidebar border border-notion-border rounded-[6px] p-[12px] px-[16px]">
                              ...
                              <div className="prose prose-notion text-[15px] leading-[1.6] text-notion-text">
                                {message.content.split('\n').map((line, i) => {
                                  if (line.startsWith('**') && line.endsWith('**')) {
                                    return (
                                      <p key={i} className="font-semibold text-notion-text my-[8px]">
                                        {line.replace(/\*\*/g, '')}
                                      </p>
                                    );
                                  }
                                  if (line.startsWith('- ')) {
                                    return (
                                      <p key={i} className="text-notion-text pl-[16px] my-[4px]">
                                        • {line.slice(2)}
                                      </p>
                                    );
                                  }
                                  if (line.startsWith('|')) {
                                    return (
                                      <p key={i} className="text-notion-text font-mono text-[13px] my-[4px]">
                                        {line}
                                      </p>
                                    );
                                  }
                                  ...
                                  return (
                                    <p key={i} className="text-notion-text my-[4px]">
                    ...
                    {isTyping && (
                      ...
                      >
                        <div className="flex gap-[4px]">
                          <span className="w-[6px] h-[6px] bg-notion-text-tertiary rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                          <span className="w-[6px] h-[6px] bg-notion-text-tertiary rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                          <span className="w-[6px] h-[6px] bg-notion-text-tertiary rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
                        </div>
              ...
              <div className="max-w-notion-chat mx-auto px-notion-massive py-[16px]">
                <div className="relative">
                  <textarea
                    ...
                    className="notion-input w-full min-h-[42px] max-h-[120px] pr-[48px] resize-none"
                    rows={1}
                  />
                  <button
                    onClick={handleSendMessage}
                    disabled={!inputValue.trim() || isTyping}
                    className={`absolute right-[8px] top-1/2 -translate-y-1/2 p-[8px] rounded-[4px] transition-colors duration-150 ${
                      inputValue.trim() && !isTyping
                        ? 'bg-notion-blue text-white hover:bg-[#1a6fcc]'
                        : 'bg-notion-border text-notion-text-tertiary cursor-not-allowed'
                    }`}
                  >
