...
const Login = () => {
  ...
  const handleDemoLogin = async () => {
    ...
    const { error } = await signIn('guest@ampm.demo', 'ampm@123');
    setIsDemoLoading(false);
    if (error) {
      toast.error('Demo account unavailable. Please run the seed script first.');
    } else {
      navigate('/dashboard');
    }
  };
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12">
        ...
        >
          ...
          <button
            ...
            className="w-full h-11 mb-4 flex items-center justify-center gap-2.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 transition-colors disabled:opacity-60 disabled:cursor-not-allowed shadow-sm"
          >
            {isDemoLoading ? (
          ...
          <p className="text-center text-xs text-muted-foreground mb-6">
            Demo is pre-loaded with repos, sprint tickets, team data, and chat history.
          </p>
