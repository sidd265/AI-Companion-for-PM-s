...
import { User, Bell, Palette, Link2, Shield, Check, Github, Trello, MessageCircle } from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const Toggle = ({ checked, onChange }: { checked: boolean; onChange: (val: boolean) => void }) => (
    <button onClick={() => onChange(!checked)} className={`relative w-12 h-7 rounded-full transition-colors duration-200 ${checked ? 'bg-primary' : 'bg-border'}`}>
      <span className={`absolute top-1 w-5 h-5 bg-white rounded-full shadow-sm transition-transform duration-200 ${checked ? 'left-6' : 'left-1'}`} />
    </button>
  );
      ...
      <div className="flex flex-col md:flex-row gap-6 md:gap-12">
        <div className="md:w-[220px] flex-shrink-0">
          <nav className="flex md:flex-col gap-1 overflow-x-auto md:overflow-x-visible pb-2 md:pb-0">
            {tabs.map((tab) => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-accent text-primary font-medium' : 'text-muted-foreground hover:bg-secondary hover:text-foreground'}`}>
            ...
            userLoading ? <ProfileSkeleton /> : (
              <div>
                ...
                <div className="flex items-start gap-6 mb-8">
                  <div className="w-24 h-24 rounded-full flex items-center justify-center text-2xl font-semibold text-white" style={{ backgroundColor: currentUser?.avatarColor ?? '#2383E2' }}>{currentUser?.initials ?? ''}</div>
                  <div>
                ...
                <div className="space-y-5">
                  <div><label className="block text-sm font-medium text-foreground mb-2">Full Name</label><input type="text" defaultValue={currentUser?.name ?? ''} className="airbnb-input w-full" /></div>
                  <div><label className="block text-sm font-medium text-foreground mb-2">Email</label><input type="email" defaultValue={currentUser?.email ?? ''} className="airbnb-input w-full" /></div>
                  <div><label className="block text-sm font-medium text-foreground mb-2">Role</label><input type="text" defaultValue={currentUser?.role ?? ''} className="airbnb-input w-full" /></div>
                  <div><label className="block text-sm font-medium text-foreground mb-2">Timezone</label>
            ...
            <div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-5 border border-border rounded-2xl">
                  <div><div className="text-sm font-medium text-foreground">Email Notifications</div><div className="text-xs text-muted-foreground">Receive updates via email</div></div>
                  <Toggle checked={emailNotifications} onChange={setEmailNotifications} />
                </div>
                <div className="flex items-center justify-between p-5 border border-border rounded-2xl">
                  <div><div className="text-sm font-medium text-foreground">Slack Notifications</div><div className="text-xs text-muted-foreground">Get notified in Slack channels</div></div>
                  <Toggle checked={slackNotifications} onChange={setSlackNotifications} />
                </div>
                <div className="flex items-center justify-between p-5 border border-border rounded-2xl">
                  <div><div className="text-sm font-medium text-foreground">Desktop Notifications</div><div className="text-xs text-muted-foreground">Browser push notifications</div></div>
            ...
            <div>
              ...
              {integrationsLoading ? (
                  ...
                  {Array.from({ length: 3 }).map((_, i) => (
                    <div key={i} className="flex items-center justify-between p-5 border border-border rounded-2xl">
                      <div className="flex items-center gap-4">
                        ...
                        <div><Skeleton className="w-16 h-4 rounded mb-1" /><Skeleton className="w-28 h-3 rounded" /></div>
                      </div>
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-5 border border-border rounded-2xl">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-gray-900 dark:bg-gray-100 rounded-xl flex items-center justify-center"><Github className="w-6 h-6 text-white dark:text-gray-900" /></div>
                      <div><div className="text-sm font-medium text-foreground">GitHub</div><div className="text-xs text-muted-foreground">{integrations?.github.connected ? `Last synced ${integrations.github.lastSync}` : 'Not connected'}</div></div>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${integrations?.github.connected ? 'bg-green-50 dark:bg-green-950 text-airbnb-success' : 'bg-secondary text-muted-foreground'}`}>{integrations?.github.connected ? 'Connected' : 'Disconnected'}</span>
                  </div>
                  <div className="flex items-center justify-between p-5 border border-border rounded-2xl">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center"><Trello className="w-6 h-6 text-white" /></div>
                      <div><div className="text-sm font-medium text-foreground">Jira</div><div className="text-xs text-muted-foreground">{integrations?.jira.connected ? `Last synced ${integrations.jira.lastSync}` : 'Not connected'}</div></div>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${integrations?.jira.connected ? 'bg-green-50 dark:bg-green-950 text-airbnb-success' : 'bg-secondary text-muted-foreground'}`}>{integrations?.jira.connected ? 'Connected' : 'Disconnected'}</span>
                  </div>
                  <div className="flex items-center justify-between p-5 border border-border rounded-2xl">
                    <div className="flex items-center gap-4">
                      <div className="w-12 h-12 bg-purple-600 rounded-xl flex items-center justify-center"><MessageCircle className="w-6 h-6 text-white" /></div>
                      <div><div className="text-sm font-medium text-foreground">Slack</div><div className="text-xs text-muted-foreground">{integrations?.slack.connected ? `Last synced ${integrations.slack.lastSync}` : 'Not connected'}</div></div>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-medium ${integrations?.slack.connected ? 'bg-green-50 dark:bg-green-950 text-airbnb-success' : 'bg-secondary text-muted-foreground'}`}>{integrations?.slack.connected ? 'Connected' : 'Disconnected'}</span>
                  </div>
                </div>
              )}
            </div>
              ...
              <div className="space-y-4">
                <div className="p-5 border border-border rounded-2xl">
                  <div className="flex items-center justify-between">
                    <div><div className="text-sm font-medium text-foreground">Export Data</div><div className="text-xs text-muted-foreground">Download all your data in JSON format</div></div>
                    <button className="airbnb-btn-secondary rounded-full py-2 px-4 text-sm">Export</button>
                  </div>
                </div>
                <div className="p-5 border border-border rounded-2xl">
                  <div className="flex items-center justify-between">
                    <div><div className="text-sm font-medium text-foreground">Clear Cache</div><div className="text-xs text-muted-foreground">Clear locally cached data</div></div>
                ...
                <div className="p-5 border border-destructive/30 rounded-2xl bg-destructive/5">
                  <div className="flex items-center justify-between">
                    <div><div className="text-sm font-medium text-destructive">Delete Account</div><div className="text-xs text-muted-foreground">Permanently delete your account and all data</div></div>
