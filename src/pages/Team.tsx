      ...
      >
        <div className="p-8">
          ...
          <div className="grid grid-cols-3 gap-3 mb-8">
            <a href={`mailto:${member.email}`} className="flex items-center gap-2 p-4 border border-border rounded-xl hover:bg-secondary transition-colors">
              <Mail className="w-4 h-4 text-muted-foreground" />
              <span className="text-sm text-foreground">Email</span>
            </a>
            <a href={`https://github.com/${member.github.slice(1)}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-4 border border-border rounded-xl hover:bg-secondary transition-colors">
          ...
          <div className="mb-8">
            ...
            <div className="flex flex-wrap gap-2">
              {member.expertise.map((skill) => (
                <span key={skill} className="airbnb-tag">{skill}</span>
              ))}
            </div>
          </div>

          <div className="mb-8">
            <h3 className="text-sm font-medium text-muted-foreground mb-3">Current Capacity</h3>
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-medium text-foreground">{Math.round(member.capacity * 100)}% utilized</span>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="p-5 bg-secondary rounded-2xl text-center">
                <div className="text-2xl font-bold text-foreground">{member.recentActivity.commits}</div>
                <div className="text-xs text-muted-foreground">Commits</div>
              </div>
              <div className="p-5 bg-secondary rounded-2xl text-center">
                <div className="text-2xl font-bold text-foreground">{member.recentActivity.reviews}</div>
                <div className="text-xs text-muted-foreground">Reviews</div>
              </div>
              <div className="p-5 bg-secondary rounded-2xl text-center">
                <div className="text-2xl font-bold text-foreground">{member.recentActivity.prsOpened}</div>
    ...
    <div className="px-4 py-6 md:px-12 md:py-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 md:mb-10 gap-4">
        <div>
          ...
          <p className="text-base text-muted-foreground">View team members, expertise, and current capacity</p>
        </div>
        <button onClick={() => setShowAddMember(true)} className="airbnb-btn-pill flex items-center gap-2 text-sm py-2.5 px-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-6 md:mb-8">
        <div className="relative flex-1 max-w-[320px]">
          ...
          <input type="text" placeholder="Search by name or role..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="airbnb-input w-full pl-11" />
        </div>
